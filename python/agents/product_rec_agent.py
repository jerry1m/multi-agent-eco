...
class ProductRecAgent(BaseAgent):
    ...
    async def _rerank(
        ...
        # 只提取有限字段（id,name,category,price,tags），避免把整个对象塞进 prompt（节省 token，减少隐私泄露风险）。
        candidate_summary = [
        ...
        # 构造langchain消息并调用LLM：使用 langchain_openai.ChatOpenAI 的 async 接口 ainvoke 
        #                             发起请求，LLM 返回对象 response，response.content 包含文本结果
        messages = [
